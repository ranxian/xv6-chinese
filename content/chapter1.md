...
本章通过第一个进程的创建，解释了 xv6 是如何开始运行的，让我们得以一窥 xv6 提供的各个主要抽象概念是如何实现和交互的。xv6 尽量使用实现普通操作的代码来建立第一个进程，希望避免单独为其撰写代码。接下来的各小节中，我们将详细探索其中的奥秘。

xv6 运行在使用 Intel 80386 及其之后（即"x86"）的处理器的 PC 平台上，因而许多底层功能（例如虚存的实现）是 x86 处理器专有的。本书假设读者已有些许在一些体系结构上进行机器级编程的经验。我们将在 x86 专有概念出现时，对其进行介绍。另外，附录 A 中简洁地描述了 PC 平台的整体框架。
