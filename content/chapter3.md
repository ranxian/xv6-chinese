...
`Alltraps`（3004）继续保存处理器的寄存器：它压入 %ds, %es, %fs, %gs, 以及通用寄存器（3005-3010)。这么做使得内核栈上压入一个 `trapframe`（中断帧） 结构体，这个结构体包含了中断发生时处理器的寄存器状态（参见图3-2）。其中处理器压入了 %ss，%esp，%eflags，%cs 和 %eip；处理器或者中断入口压入了一个错误码；`alltraps`压入了剩余的内容。中断帧包含了当内核返回当前进程时，恢复用户态下处理器寄存器所需要的全部信息，所以处理器可以恰如中断开始时那样继续执行。回顾一下第二章，`userinit`通过手动建立中断帧来达到这个目标（参见图1-3）。
